...
def search(search_text):
    ...
    try:
        ...
        WebDriverWait(driver, 60).until(
            ec.invisibility_of_element((By.CLASS_NAME, "react-loading-skeleton"))
        )
        driver.find_element_by_class_name("button").click()
        WebDriverWait(driver, 60).until(
            ec.invisibility_of_element(
                (By.CLASS_NAME, "css-1vmnjpn-skeletonStyles-Skeleton")
            )
        )
        if cur_url == driver.current_url:
            error = driver.find_element_by_css_selector(
                ".error span:nth-child(" "1) > span"
            ).text
            raise Exception(f"Не указан параметр:{error}")
        element = driver.find_element_by_xpath(
            "/html/body/div[2]/div/main/div[1]/div[2]/div[2]/div/div[1]/div[1]/div[1]/div/div"
        ).text
        data_dict["Марка"] = element
        print(element)
        element = driver.find_element_by_xpath(
            "/html/body/div[2]/div/main/div[1]/div[2]/div[2]/div/div[1]/div[1]/div[2]/div/div"
        ).text
        data_dict["Модель"] = element
        print(element)
        elements = driver.find_elements_by_css_selector(
            '[class = "form-el__content valid filled"]'
        )
        for el in elements:
            ...
            b = (
                el.find_element_by_css_selector("input")
                .get_attribute("value")
                .replace(" ", "")
            )
            print(f"{a}:{b}")
